...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif ($updateData['email'] !== $user['email']) {
        // Cek apakah email sudah ada (jika diganti)
    ...
    // Validasi role
    if (empty($updateData['role']) || !in_array($updateData['role'], ['admin', 'customer'])) {
        $errors[] = 'Role tidak valid';
    }

    // Jika pengguna saat ini mengupdate dirinya sendiri, tidak boleh mengganti role dari admin
    if ($userId == $_SESSION['user_id'] && $user['role'] == 'admin' && $updateData['role'] != 'admin') {
        $errors[] = 'Anda tidak dapat mengubah role Anda sendiri dari admin';
    }

    // Jika tidak ada error, update pengguna
    if (empty($errors)) {
        ...
        // Jika ada error, terapkan data dari form
        $formData = $updateData;
    }
}
...
<div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-semibold text-gray-900">Edit Pengguna: <?php echo $user['name']; ?></h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="password" id="password" name="password"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <p class="text-xs text-gray-500 mt-1">Biarkan kosong jika tidak ingin mengubah password. Minimal 6 karakter.</p>
                </div>
                ...
                <div>
                    ...
                    <select id="role" name="role" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                        <?php echo ($userId == $_SESSION['user_id'] && $user['role'] == 'admin') ? 'disabled' : ''; ?>>
                    ...
                    <?php if ($userId == $_SESSION['user_id'] && $user['role'] == 'admin'): ?>
                        <input type="hidden" name="role" value="admin">
                        <p class="text-xs text-gray-500 mt-1">Anda tidak dapat mengubah role Anda sendiri dari admin.</p>
                    <?php endif; ?>
                </div>
