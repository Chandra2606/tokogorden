...
$formData = [
    ...
    'image' => $product['image'],
    'is_featured' => isset($product['is_featured']) ? $product['is_featured'] : 0
];
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (!isset($errors['image'])) {
            $newImage = 'product_' . time() . '_' . uniqid() . '.' . $fileExtension;
            ...
            if (move_uploaded_file($fileTmpName, $uploadPath)) {
                if (!empty($product['image'])) {
                    $oldImagePath = '../../assets/images/products/' . $product['image'];
    ...
    if (empty($errors)) {
        ...
        $result = $productModel->update($product_id, $productData);

        if ($result) {
            $_SESSION['success_message'] = 'Produk berhasil diperbarui';
            redirect('admin/products/index.php');
        } else {
            $_SESSION['error_message'] = 'Gagal memperbarui produk. Silakan coba lagi.';
        }
    }
}
                ...
                <div>
                    ...
                    <?php if (!empty($formData['image'])): ?>
                        <div class="mt-2">
                            ...
                            <img src="../../assets/images/products/<?php echo htmlspecialchars($formData['image']); ?>"
                                alt="<?php echo htmlspecialchars($formData['name']); ?>"
